...
st.write("Thisis used to add new faces to existing criminal dataset")

menu = ["Adding","Deleting", "Adjusting"]
choice = st.sidebar.selectbox("Options",menu)
if choice == "Adding":
    name = st.text_input("Name",placeholder='Enter Criminal name')
    id = st.text_input("ID",placeholder='Enter Criminal id')
    ...
    if upload == "Upload":
        ...
        if uploaded_image is not None:
            ...
            if submit_btn:
                if name == "" or id == "":
                    ...
                    if ret == 1: 
                        st.success("Criminal Sucessfully Added To the Data")
                    elif ret == 0: 
                        st.error("Criminal ID already exists")
                    elif ret == -1: 
        ...
        if img_file_buffer is not None:
            ...
            if submit_btn: 
                if name == "" or id == "":
                    st.error("Please enter criminal name and criminal ID")
                else:
    ...
    def del_btn_callback(id):
        deleteOne(id)
        st.success("Criminal deleted from Dataset ")
    ...
    if submit_btn:
        ...
        if name == None and image == None:
            st.error("Criminal ID does not exist")
        else:
            st.success(f"Name of Criminal with ID {id} is: {name}")
            st.warning("Please check the image below to make sure you are deleting the right Criminal Data")
            st.image(image)
    ...
    def form_callback(old_name, old_id, old_image, old_idx):
        ...
        if ret == 1: 
            st.success("Criminal Detail  Added")
        elif ret == 0: 
            st.error("Criminal ID already exists")
        elif ret == -1: 
            st.error("There is no face in the picture")
    ...
    if submit_btn:
        ...
        if old_name == None and old_image == None:
            ...
            with st.form(key='my_form'):
                st.title("Adjusting Criminal info")
